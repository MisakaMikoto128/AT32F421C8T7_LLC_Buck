...
/**
 * @brief 计算 Modbus 协议的 CRC16 校验值
 *
 * @param _pBuf 数据缓冲区指针
 * @param _usLen 数据长度
 * @return uint16_t 计算后的 CRC16 值，返回值已高低字节交换，适合直接发送
 */
uint16_t CRC16_Modbus(const uint8_t *_pBuf, uint16_t _usLen)
{
    return HDL_CRC16_Modbus(_pBuf, _usLen);
}

/**
 * @brief 使用硬件 CRC 单元计算 CRC16 (Modbus)
 *
 * @param crc 初始 CRC 值
 * @param _pBuf 数据缓冲区指针
 * @param _usLen 数据长度
...
uint16_t CRC16_Modbus_With(uint16_t crc, const uint8_t *_pBuf, uint16_t _usLen)
{
    return HDL_CRC16_Modbus_With(crc, _pBuf, _usLen);
}
#endif
...
// CRC16 CCITT-FALSE 查找表（多项式：0x1021）
...
{
    uint16_t crc = 0xFFFF; // CCITT-FALSE 初始值为 0xFFFF
    
    while (_usLen--)
    {
        // 使用高字节进行查表运算
        uint8_t tbl_idx = ((crc >> 8) ^ *_pBuf++) & 0xFF;
        crc = (crc << 8) ^ crc16_ccitt_false_table[tbl_idx];
    }
