 ...
 * @return int Returns 1 if output was limited to max, -1 if limited to min, 0 otherwise.
 */
int Inc_PID_Q32_Update_AddDeltaF1(pInc_PID_Q32_t self)
{
    int res       = 0;
    int32_t delta = 0;
    int64_t F     = 0;
    ...
    if (self->iError > 0) {
        self->maxDelta = 4096*4 / self->iError;
        self->minDelta = -self->maxDelta;
    } else if (self->iError == 0) {
        self->maxDelta = INT32_MAX;
        self->minDelta = -self->maxDelta;
    } else if (self->iError < 0) {
        self->minDelta = 4096*4 / self->iError;
        self->maxDelta = -self->minDelta;
    }
