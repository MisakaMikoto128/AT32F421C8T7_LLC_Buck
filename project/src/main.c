...
// (300-0)  -> (50%-0%) -> (150-0)
// LLC 输入电流安全上限
#define LLC_INPUT_CURRENT_UPPER_LIMIT 5.2f // 5A。
// LLC 输入电流下限
#define LLC_INPUT_CURRENT_LOWER_LIMIT 0.0f // 0A
// LLC 输入电流限制
#define LLC_INPUT_CURRENT_OC_LIMIT (4.9f) // 4A。
...
int main(void)
{
    ...
    LLC_CURR_DEBUG_ADC_VALUE       = llc_curr_to_adc_value(LLC_INPUT_CURRENT_OC_LIMIT);
    // 初始化PID控制器
    user_pid_init();
    // 初始LLC频率为最高频率
    ...
    while (1) {
        ...
        if (TICK_COUNT_VALUE - ms_rec > 1000 * 6) {
            ...
            ULOG_INFO("LLC V: target=%fV, adc=%u, calc=%fV, err:%d,iFv:%f,iF:%8d, f:%8f",
                      llc_volt_target,
                      filtered_adc[ADC_V_LLC_RANK_IDX],
                      votlage_debug[ADC_V_LLC_RANK_IDX],
                      llc_volt_pid.iError,
                      ((llc_volt_pid.iF >> PID_SHIFT) - adc_buffer_init[ADC_IIN_RANK_IDX]) * adc_to_target_scale[ADC_IIN_RANK_IDX],
...
#define FILTER_SHIFT 3 // 相当于除以8的滤波系数
    ...
    switch (stage) {
        ...
        case 2:
            ...
            // 频率越低，PID最大增量越小
            if (iF > (((TMR1_CLK_FREQ / 240000UL) - 1) << PID_SHIFT_14)) {
                Inc_PID_Q32_Set_DeltaLimit(&llc_curr_freq_pid, INT32_MAX, INT32_MIN);
            } else {
                Inc_PID_Q32_Set_DeltaLimit(&llc_curr_freq_pid, llc_curr_freq_pid_limit, -llc_curr_freq_pid_limit);
            }
